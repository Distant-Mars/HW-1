...
/* ------初始化界面和一些数据,temp就是取每一个块的id，然后修改他们的css样式,修改了大小和位置哦------*/
function newGame() {
	for (var i = 0; i < 5; i++) {
		var temp = $("#contain"+i);
		temp.css('width', '0px');
...
/*-------点击菜单打开（效果就是菜单变长了~其实只是把它长度变长了~用jquery的动画效果就是拉长~）---------*/
function onTouch() {
	if (clickbool) {
		for (var i = 0; i < 6; i++) {
			var inin = $("#contain"+i);
...
/*----分别对每个糕点图片鼠标悬停设置相应行为，具体就是鼠标过来了就变大，走开了就变小，然后对应的图片也变大居中-----
------不太会批量设置,辛苦观看了QAQ每个都一个意思啦，只是对应的图片要定位位置都不同所以要一个个写------*/
function over0() {
	if (movebool) {
		var inin = $("#contain0");
		inin.animate({
